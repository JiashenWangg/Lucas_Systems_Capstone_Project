...
│       ├── WH_WCxx_seqLB.json       # lower bound model
│       ├── WH_WCxx_seqUB.json       # upper bound model
...
`[internal]` Incrementally updates saved XGBoost models with new completed pick data. Run after new picks have been recorded. No historical data is stored between runs — the model file itself accumulates the learned patterns.
...
When daily batches are small relative to the full training history, they are weighed less in the model. We include a learning rate that is variable and depends on the size comparison of the input and already saved data. The adaptive learning rate policy is as follows.

$\frac{m}{n}\leq0.10 \implies 0.005$

$0.10<\frac{m}{n}\leq0.25 \implies 0.008$

$0.25<\frac{m}{n}\leq0.50 \implies 0.015$

$\frac{m}{n}\>0.5 \implies 0.03$
