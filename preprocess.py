...
def validate_inputs(base_path: Path, wh_name: str):
    ...
    required_files = [
        ...
        csv_path / f"{wh_name}_Products.csv",
...
def load_tables(base_path, wh_name, column_names):
    ...
    tables = {
        ...
        f"{wh_name}_Products": csv_path / f"{wh_name}_Products.csv",
    }

    dfs = {}
    for name, fp in tables.items():
        logger.info(f"Loading {fp}")
        num_rows = len(pd.read_csv(fp, header=0, nrows=1).columns)
        if num_rows == len(column_names[name]):
            dfs[name] = pd.read_csv(fp, header=0, names=column_names[name])
        else:
            logger.warning(f"Not all columns used when loading {fp}")
            dfs[name] = pd.read_csv(fp, header=0, names=column_names[name][:num_rows])
...
def compute_distances(df, dist_long):
    ...
    return df.rename(columns={"distance": "Travel_Distance"}).drop(
        columns=["FromLoc", "ToLoc", "Aisle2", "Bay2", "Prev_Aisle2", "Prev_Bay2"])


# -----------------------------
# EXPORT
# -----------------------------
def export_outputs(base_path, wh_name, df_detailed):
    output_dir = base_path / "processed"
    output_dir.mkdir(parents=True, exist_ok=True)

    df_detailed.to_parquet(output_dir / f"{wh_name}_detailed.parquet", index=False)

    logger.info(f"Exported file to {output_dir}")
