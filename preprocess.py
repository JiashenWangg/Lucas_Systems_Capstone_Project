...
dfs[activity_key]["ProductID"] = pd.to_numeric(
    dfs[activity_key]["ProductID"], errors="coerce"
).astype("Int64")
dfs[activity_key]["Quantity"] = pd.to_numeric(
    dfs[activity_key]["Quantity"], errors="coerce"
).astype("Int64")
dfs[activity_key]["LocationID"] = pd.to_numeric(
    dfs[activity_key]["LocationID"], errors="coerce"
).astype("Int64")
dfs[activity_key]["Timestamp"] = pd.to_datetime(
    dfs[activity_key]["Timestamp"], errors="coerce"
)
...
dfs[locations_key]["LocationID"] = pd.to_numeric(
    dfs[locations_key]["LocationID"], errors="coerce"
).astype("Int64")
for col in ["Bay", "Level", "Slot"]:
    dfs[locations_key][col] = pd.to_numeric(
        dfs[locations_key][col], errors="coerce"
    ).astype("Int64")

# Clean product data for merging
dfs[products_key]["ProductID"] = pd.to_numeric(
    dfs[products_key]["ProductID"], errors="coerce"
).astype("Int64")
dfs[products_key] = dfs[products_key][
    ["ProductID", "ProductCode", "UnitOfMeasure", "Weight", "Cube"]
]
...
print(
    f"98th Percentile Threshold: {threshold:.2f} seconds ({(threshold/60):.2f} minutes)"
)
df_work.loc[df_work["Time_Delta_sec"] > threshold, "Time_Delta_sec"] = np.nan
print(
    f"Number of rows with Time Deltas: {len(df_work[df_work['Time_Delta_sec'].notnull()])}"
)
...
df_joined = df_joined.merge(
    Locations[["LocationID", "Aisle", "Bay", "Level", "Slot"]].rename(
        columns={
...
df_detailed["Aisle2"] = (
    pd.to_numeric(df_detailed["Aisle"], errors="coerce")
    .astype("Int64")
    .astype(str)
    .str.zfill(2)
)
df_detailed["Bay2"] = (
    pd.to_numeric(df_detailed["Bay"], errors="coerce")
    .astype("Int64")
    .astype(str)
    .str.zfill(2)
)
df_detailed["Prev_Aisle2"] = (
    pd.to_numeric(df_detailed["Prev_Aisle"], errors="coerce")
    .astype("Int64")
    .astype(str)
    .str.zfill(2)
)
df_detailed["Prev_Bay2"] = (
    pd.to_numeric(df_detailed["Prev_Bay"], errors="coerce")
    .astype("Int64")
    .astype(str)
    .str.zfill(2)
)

# Create LocKey and PrevLocKey for merging with distance matrix
if ":" in dist_long["FromLoc"].iloc[0]:
    df_detailed["LocKey"] = df_detailed["Aisle2"] + ":" + df_detailed["Bay2"]
    df_detailed["PrevLocKey"] = (
        df_detailed["Prev_Aisle2"] + ":" + df_detailed["Prev_Bay2"]
    )
else:
    SUFFIX = "|||"
    df_detailed["LocKey"] = df_detailed["Aisle2"] + "|" + df_detailed["Bay2"] + SUFFIX
    df_detailed["PrevLocKey"] = (
        df_detailed["Prev_Aisle2"] + "|" + df_detailed["Prev_Bay2"] + SUFFIX
    )
...
df_detailed = (
    ...
    .rename(columns={"distance": "Travel_Distance"})
    .drop(columns=["FromLoc", "ToLoc"])
)
...
df_detailed.to_parquet(output_dir / f"{warehouse_name}_detailed.parquet", index=False)
Activity_prepped.to_parquet(
    output_dir / f"{warehouse_name}_activity_prepped.parquet", index=False
)
df_joined.to_parquet(output_dir / f"{warehouse_name}_joined.parquet", index=False)
